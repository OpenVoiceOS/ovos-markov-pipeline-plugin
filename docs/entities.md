...
BIO tags, and the observations are the words. Given a new tokenized utterance,
the Viterbi algorithm finds the most likely tag sequence, which is then parsed
back into a `{slot_name: value}` dict.
...
---
[← Domain pipeline](domain_engine.md) · [Home](index.md) · [Calibration →](calibration.md)
