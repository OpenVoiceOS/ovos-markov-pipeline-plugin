...
Local implementation of ``expand_template`` so the plugin does not need a
runtime dependency on ``ovos-utils`` just for two regexes.  Mirrors the
semantics of ``ovos_utils.bracket_expansion``.
"""
import itertools
import re
